...
while not rospy.is_shutdown():
	...
	y = (int(x[0]),int(x[1]),int(x[2]),int(x[3]),int(x[4]),int(x[5]),int(sensor_state[2]),int(sensor_state[3]),int(sensor_state[4]),int(sensor_state[5])) #repack IR&Cliff Sensor into array
